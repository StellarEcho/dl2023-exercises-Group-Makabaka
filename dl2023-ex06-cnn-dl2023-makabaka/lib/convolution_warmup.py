...
def pad_array(X: np.ndarray, pad: Tuple[int, int]) -> np.ndarray:
    ...
    result = np.pad(X, ((0, 0), (0, 0), (pad[0], pad[0]), (pad[1], pad[1])), mode='constant')
    # END TODO ################
    return result
...
def dot_product_single_filter(X: np.ndarray, F: np.ndarray, index: Tuple[int, int]) -> float:
    ...
    # START TODO ################
    # raise NotImplementedError
    fChannel, fHeight, fWeight = F.shape
    startHeight, startWidth = index
    X_part = X[:, startHeight:startHeight + fHeight, startWidth:startWidth + fWeight]
    result = np.sum(X_part * F)
    # END TODO ################
...
def dot_product_single_filter_with_batches(X: np.ndarray, F: np.ndarray, index: Tuple[int, int]) -> np.ndarray:
    ...
    for i in range(len(X)):
        X_part = X[i, :, startHeight:startHeight + fHeight, startWidth:startWidth + fWeight]
        result[i] = np.sum(X_part * F)
    # END TODO ################
...
def dot_product_multiple_filters_with_batches(X: np.ndarray, filters: np.ndarray, index: Tuple[int, int]) -> np.ndarray:
    ...
    _, _, fHeight, fWeight = filters.shape
    startHeight, startWidth = index
    for i in range(len(X)):
        for j in range(len(filters)):
            X_part = X[i, :, startHeight:startHeight + fHeight, startWidth:startWidth + fWeight]
            result[i, j] = np.sum(X_part * filters[j])
    # END TODO ################
