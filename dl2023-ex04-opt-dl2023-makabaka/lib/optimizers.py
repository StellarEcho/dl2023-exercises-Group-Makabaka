...
class Adam(Optimizer):
    ...
    def step(self) -> None:
        ...
        for p in self._params:
            g = p.grad
            p.state_dict["s"] = self.betas[0] * p.state_dict["s"] + (1 - self.betas[0]) * g
            p.state_dict["r"] = self.betas[1] * p.state_dict["r"] + (1 - self.betas[1]) * g * g

            m_h = p.state_dict["s"] / (1 - self.betas[0] ** self.t)
            v_h = p.state_dict["r"] / (1 - self.betas[1] ** self.t)

            p.data = p.data - (self.lr / (np.sqrt(v_h) + self.eps)) * m_h
        # END TODO###################
