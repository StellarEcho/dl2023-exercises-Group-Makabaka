...
class DNGO(nn.Sequential):
    ...
    def fit_blr_model(self, mu_pre: np.array, Sigma_pre: np.array,
        ...
        X = self.last_hidden_layer_features(x_train)
        # X = np.concatenate([X, np.ones((X.shape[0], 1))], axis=1)
        # X = torch.tensor(X, dtype=torch.float32)
        # Y = y_train
        # Sigma_pre_inv = np.linalg.inv(Sigma_pre)
        # Sigma_post_inv = Sigma_pre_inv + (1.0/noise) * X.T @ X
        # Sigma_post = np.linalg.inv(Sigma_post_inv)
        # mu_post = Sigma_post @ (Sigma_pre_inv @ mu_pre + (1.0 / noise) * X.T @ Y)
        self.blr = BLR(mu_pre, Sigma_pre, noise, bias=True)
        self.mu_post, self.Sigma_post = self.blr.linreg_bayes(X, y_train)
    ...
    def predict_mean_and_std(self, x: torch.Tensor) -> Tuple[np.array, np.array]:
        ...
        # raise NotImplementedError
        X = self.last_hidden_layer_features(x)
        x_mean, x_std = self.blr.posterior_predictive(X)
        # END TODO ########################
        return x_mean, x_std
...
class EnsembleFeedForward:
    ...
    def predict_mean_and_std(self, x: torch.Tensor) -> Tuple[np.array, np.array]:
        ...
        mean = torch.mean(preds, dim=2, dtype=torch.float32)
