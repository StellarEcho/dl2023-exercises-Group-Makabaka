...
def train_noise_removal_model(hidden_size=40, shift=10, lr=0.01, num_epochs=101, batch_size=10, plot=True,
    ...
    for epoch in range(num_epochs):
        ...
        for b in range(NUM_FUNCTIONS_TRAIN // batch_size):
            optimizer.zero_grad()
            batch = noisy_train_sequences[b * batch_size:(b + 1) * batch_size]
            output = model(batch)
            label = train_sequences[b * batch_size:(b + 1) * batch_size]
...
def run_hpo() -> None:
    ...
    for b in batch_size:
        train_noise_removal_model(hidden_size=40, shift=10, lr=0.01, num_epochs=100, batch_size=b, reproduce=True)
        print("Batch Size: ", b)

    num_epochs = range(100, 500, 100)
    for n in num_epochs:
        train_noise_removal_model(hidden_size=40, shift=10, lr=0.01, num_epochs=n, batch_size=10, reproduce=True)
        print("Num Epochs: ", n)

    shift = range(2, 15, 1)
    for s in shift:
        train_noise_removal_model(hidden_size=40, shift=s, lr=0.01, num_epochs=100, batch_size=10, reproduce=True)
        print("Shift: ", s)

    hidden_size = range(10, 60, 10)
    for h in hidden_size:
        train_noise_removal_model(hidden_size=h, shift=10, lr=0.01, num_epochs=100, batch_size=10, reproduce=True)
        print("Hidden Size: ", h)
    # END TODO
