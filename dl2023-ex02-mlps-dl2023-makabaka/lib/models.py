...
def create_2unit_net() -> Sequential:
...
def create_3unit_net() -> Sequential:
    ...
    model.parameters()[0].data = np.array([[3.21, -2.34, 0], [3.21, -2.34, 0]])  # W1
    model.parameters()[1].data = np.array([-3.21, 2.34, 0])  # b1
    model.parameters()[2].data = np.array([[3.19, -2.68], [4.64, -3.44], [0, 0]])  # W2
    model.parameters()[3].data = np.array([-4.08, 4.42])  # b2
    # END TODO ##################
...
def run_model_on_xor(model: Module, verbose: bool = True) -> Tuple[np.ndarray, float]:
    ...
    # raise NotImplementedError
    Y_onehot = one_hot_encoding(y, num_classes=2)
    # END TODO ##################
