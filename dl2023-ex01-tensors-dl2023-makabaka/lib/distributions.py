...
def plot_clt(n_repetitions: int = 1024, sample_sizes: Tuple[int, ...] = (1, 16, 64, 1024), plot=True) \
    ...
    for nrow, n_samples in enumerate(sample_sizes):
        for ncol, (dist_name, dist_fn) in enumerate(distributions.items()):
            ...
            # samples = [dist_fn(n_samples) for _ in range(n_repetitions)]
...
def std_normal(n_samples: int = 1) -> np.ndarray:
    ...
    samples = np.random.uniform(-b, b, (n_samples, n_uniform_samples))

    # std_normal_samples = np.mean(uniform_samples)
...
def normal(loc: float = 0.0, scale: float = 1.0, n_samples: int = 1) -> np.ndarray:
    ...
    # std_normal_samples = std_normal(n_samples)
    # normal_samples = loc + scale * std_normal_samples
    b = np.sqrt(3) * scale
    samples = np.random.uniform(loc-b, loc+b, size=(n_samples,))
    return samples
    # raise NotImplementedError
    # END TODO #############
