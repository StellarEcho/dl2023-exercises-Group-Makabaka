...
def show_attention_on_input_image(net) -> None:
    ...
    for i, (data, target) in enumerate(dataloader):
        if i in [
            ...
            maxNorm = torch.max(norm)
            minNorm = torch.min(norm)
            normalized_gradient = (norm - minNorm)/(maxNorm - minNorm)
            # END TODO #############
    ...
    for i, (data, target) in enumerate(dataloader):
        if i in [
            ...
            try:
                ...
                # raise NotImplementedError
                gradSign = torch.sign(gradient)
                adversarial_example = orig_data + 0.1 * gradSign.to(device)
                # END TODO #############
