...
def train_models() -> Tuple[Dict[str, Module], Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]]:
    ...
    # create loss, optimizer and train the model
    # raise NotImplementedError
    l2R = L2Regularization(lambda_l2, params)
    # cross_entropy_l2 = CrossEntropyLoss()
    cross_entropy_l2 = RegularizedCrossEntropy(l2R)
    optimizer = SGD(models["l2"].parameters(), lr=learning_rate, momentum=momentum)
    # training_result = train(models["l2"], cross_entropy_l2 + l2R, optimizer, x_train, y_train,
    #                         x_val, y_val, num_epochs=num_epochs, batch_size=batch_size)
    training_result = train(models["l2"], cross_entropy_l2, optimizer, x_train, y_train,
                            x_val, y_val, num_epochs=num_epochs, batch_size=batch_size)
    # END TODO ################
    # write back result
    ...
    # create loss, optimizer and train the model
    # raise NotImplementedError
    l1R = L1Regularization(lambda_l1, params)
    cross_entropy_l1 = RegularizedCrossEntropy(l1R)
    optimizer = SGD(models["l1"].parameters(), lr=learning_rate, momentum=momentum)
    # training_result = train(models["l1"], cross_entropy_l1 + l1R, optimizer, x_train, y_train,
    #                         x_val, y_val, num_epochs=num_epochs, batch_size=batch_size)
    training_result = train(models["l1"], cross_entropy_l1, optimizer, x_train, y_train,
                            x_val, y_val, num_epochs=num_epochs, batch_size=batch_size)
    # END TODO ################
    # write back result
    ...
    # create loss, optimizer and train the model
    # raise NotImplementedError
    cross_entropy_dp = CrossEntropyLoss()
    optimizer = SGD(models["dropout"].parameters(), lr=learning_rate, momentum=momentum)
    training_result = train(models["dropout"], cross_entropy_dp, optimizer, x_train, y_train,
                            x_val, y_val, num_epochs=num_epochs, batch_size=batch_size)
    # END TODO ################
    # write back result
