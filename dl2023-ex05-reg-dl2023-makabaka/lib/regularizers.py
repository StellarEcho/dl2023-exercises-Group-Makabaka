...
class Dropout(Module):
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        self.mask = np.random.binomial(1, 1 - self.p_delete, x.shape) / (1 - self.p_delete)
        return x * self.mask
        # END TODO ################
...
class L1Regularization(Module):
    ...
    def forward(self) -> float:
        ...
        # START TODO ################
        # raise NotImplementedError
        return self.lambd * sum(np.sum(np.abs(p.data)) for p in self.params)
        # END TODO ################
    ...
    def backward(self, _=None) -> None:
        ...
        for p in self.params:
            p.grad += self.lambd * np.sign(p.data)
        # END TODO ################
...
class L2Regularization(Module):
    ...
    def forward(self) -> float:
        ...
        # START TODO ################
        # raise NotImplementedError
        return self.lambd * sum(np.sum(p.data ** 2) for p in self.params) / 2
        # END TODO ################
