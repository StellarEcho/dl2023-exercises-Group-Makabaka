...
class VAE(nn.Module):
    ...
    def __init__(self, in_channels: int = 1, in_height: int = 28, in_width: int = 28, hidden_size: int = 100,
        ...
        self.encode_input = nn.Linear(input_size, hidden_size)
        self.encode_mu = nn.Linear(hidden_size, latent_size)
        self.encode_logvar = nn.Linear(hidden_size, latent_size)

        self.decode_latent = nn.Linear(latent_size, hidden_size)
        self.decode_output = nn.Linear(hidden_size, input_size)
        # END TODO ########################
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # START TODO ########################
        hidden = self.activation(self.encode_input(x))

        mu = self.encode_mu(hidden)
        logvar = self.encode_logvar(hidden)
        # END TODO ########################
        return mu, logvar
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        hidden_representation = self.activation(self.decode_latent(z))
        output = self.activation_output(self.decode_output(hidden_representation))
        # END TODO ########################
