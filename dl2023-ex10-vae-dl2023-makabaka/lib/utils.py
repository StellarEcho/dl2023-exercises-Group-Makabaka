...
def add_noise(batch, t, baralphas):
    ...
    alpha = 1.0 - baralphas[t]
    alpha_bar_t = torch.cumprod(1.0 - baralphas[:t + 1], dim=0)
    # alpha_bar_t = alpha_bar_t.unsqueeze(1)
    # alpha_bar_t = torch.cumprod(1.0 - baralphas[:t], dim=0).unsqueeze(1)
    noised = alpha_bar_t * batch + torch.sqrt(1.0 - alpha_bar_t) * eps
    if noised is None:
        raise NotImplementedError(
